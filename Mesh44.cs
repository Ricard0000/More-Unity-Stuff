...
public class Mesh44 : MonoBehaviour
{
    ...
    public static GameObject MakeDiscreteProceduralGrid(float Pos1, float Pos2, float Pos3, int N, int N1, Material mat, bool collider)
    {
        ...
        {
            temp = vertices[i].x;
            vertices[i].x = Mathf.Cos(Mathf.PI / 2f) * vertices[i].x + Mathf.Sin(Mathf.PI / 2f) * vertices[i].z;
            vertices[i].z = -Mathf.Sin(Mathf.PI / 2f) * temp + Mathf.Cos(Mathf.PI / 2f) * vertices[i].z;
        }
        ...
        {
            vertices[i] = vertices[i] * meshScale + new Vector3(-3.58f - 0.16f, 1.068f + 0.181f, -2.696f - 0.021f + 0.011f);
        }
