...
public class ArchBundel3 : MonoBehaviour
{
    Mesh mesh;
    Vector3[] vertices;
    Vector2[] uvs;
    ...
    void MakeDiscreteProceduralGrid()
    {
        ...
        for (int j = 0; j < N_cir; j++)
        {
            ...
            {
                x = -1.0f * c1 + 2.0f * c1 * i / (2.0f * N);
                y = R * Mathf.Sin(t) + 2 * R + Arch_eq(x, c1, c2);
                z = R * Mathf.Cos(t) + R;
                vertices[i + j * (2 * N + 1) + next_vert] = new Vector3(x, y, z);
                uvs[i + j * (2 * N + 1) + next_vert] = new Vector2(0.5f + 0.5f / c1 * x, tt);
            }
        }
        next_vert = 2 * (2 * N + 2 + (2 * N - 1) + (2 * N + 1) * (N_cir - 2)) + 2;
        for (int j = 0; j < N_cir; j++)
        {
            t = -PI * 0.5f + PI * 0.5f * (j) / (N_cir - 1);
            tt = (float)j / (float)N_cir;
            ...
            {
                x = -1.0f * c1 + 2.0f * c1 * i / (2.0f * N);
                y = R * Mathf.Sin(t) + 3 * R + Arch_eq(x, c1, c2);
                z = R * Mathf.Cos(t) + 2 * R;
                vertices[i + j * (2 * N + 1) + next_vert] = new Vector3(x, y, z);
                uvs[i + j * (2 * N + 1) + next_vert] = new Vector2(0.5f + 0.5f / c1 * x, tt);
            }
        }
        next_vert = 3 * (2 * N + 2 + (2 * N - 1) + (2 * N + 1) * (N_cir - 2)) + 3;
        for (int i = 0; i < 2 * N + 1; i++)
        {
            x = -1.0f * c1 + 2.0f * c1 * i / (2.0f * N);
            y = 3 * R + Arch_eq(x, c1, c2);
            z = 0f;
            vertices[i + next_vert] = new Vector3(x, y, z);
            uvs[i + next_vert] = new Vector2(0.5f + 0.5f / c1 * x, 1f);
        }
