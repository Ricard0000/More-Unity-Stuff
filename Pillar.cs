...
public class Pillar : MonoBehaviour
{
    ...
    public static GameObject MakeDiscreteProceduralGrid(float Pos1, float Pos2, float Pos3, float c1, float c2, float c3, float c4, float s, float t, float delta_theta, int N, int NN, bool collider, Material mat, int rot)
    {
        ...
        */
        vertices = new Vector3[7 * N];
        triangles = new int[6 * N + 6 * N + 6 * N + 6 * N + 6 * N + 6 * N + 6 * N];
        uvs = new Vector2[7 * N];
        ...
        for (int i = 0; i < N; i++)
        {
            vertices[i] = new Vector3(r * Mathf.Cos(2f * PI * i / (float)(N-1)) + center[0], center[1] - 0.02f, r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }
        int nextVert = N;
        for (int i = 0; i < N; i++)
        {
            vertices[nextVert + i] = new Vector3(1.15f * r * Mathf.Cos(2f * PI * i / (float)(N - 1)) + center[0], center[1] - 0.02f - dy * 0.5f, 1.15f * r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }

        nextVert = nextVert + N;
        for (int i = 0; i < N; i++)
        {
            vertices[nextVert + i] = new Vector3(1.35f * r * Mathf.Cos(2f * PI * i / (float)(N - 1)) + center[0], center[1] - 0.02f - dy * 1.15f, 1.35f * r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }

        nextVert = nextVert + N;
        for (int i = 0; i < N; i++)
        {
            vertices[nextVert + i] = new Vector3(1.45f * r * Mathf.Cos(2f * PI * i / (float)(N - 1)) + center[0], center[1] - 0.02f - dy * 1.75f, 1.45f * r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }

        nextVert = nextVert + N;
        for (int i = 0; i < N; i++)
        {
            vertices[nextVert + i] = new Vector3(1.45f * r * Mathf.Cos(2f * PI * i / (float)(N - 1)) + center[0], center[1] - 0.02f - dy * 2.25f, 1.45f * r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }

        nextVert = nextVert + N;
        for (int i = 0; i < N; i++)
        {
            vertices[nextVert + i] = new Vector3(1.4f * r * Mathf.Cos(2f * PI * i / (float)(N - 1)) + center[0], center[1] - 0.02f - dy * 3f, 1.4f * r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }

        nextVert = nextVert + N;
        for (int i = 0; i < N; i++)
        {
            vertices[nextVert + i] = new Vector3(1.1f * r * Mathf.Cos(2f * PI * i / (float)(N - 1)) + center[0], center[1] - 0.02f - dy * 5f, 1.1f * r * Mathf.Sin(2f * PI * i / (float)(N - 1)) + center[2]);
        }
        ...
        for (int i = 0; i < N - 1; i++)
        {
            int i3 = 3 * i;
            triangles[nextTri + i3] = 5 * N + i;
            triangles[nextTri + i3 + 1] = 5 * N + i + 1;
            triangles[nextTri + i3 + 2] = 6 * N + i;
        }
        nextTri = nextTri + 3 * N;
        for (int i = 0; i < N - 1; i++)
        {
            int i3 = 3 * i;
            triangles[nextTri + i3] = 6 * N + i;
            triangles[nextTri + i3 + 1] = 5 * N + i + 1;
